...
// ----------------------------------------------------
void Creature::Rest(const vector<string>& args) 
{

	if (parent->GetNameRoom() == "SmallRoom") {
		cout << "\n your stats has ben restore \n";
		hit_points = 25;
	}
	else {
...
// ----------------------------------------------------
// if a creature dies, drop all he has
void Creature::Die()
{
	if(PlayerInRoom()){
		cout << name << " dies.\n";
	...
	if (items.size() == 0)
	{
		...
		for (list<Entity*>::const_iterator it = items.begin(); it != items.cend(); ++it)
		{
			...
			if (*it == weapon) {
				...
				if (PlayerInRoom()) {
					cout << name << " drops " << (*it)->name << "...\n";
					Item* item = (Item*)Find((*it)->name, ITEM);
					item->ChangeParentTo(parent);
				}
			}
